...
that is used in the online phase to provide feedback on the decoding results.
...
At the beginning of the road segment, a symbol (the cue) is depicted
...
We included empty roads where subjects have no additional tasks 
...
After crossing the finish line the board with the predicted target is rendered
...
on each road with the average fraction of the targets of 0.25 in total.

\subsubsection*{Symbols in the online phase.}
To allow equal accumulation for each symbol by keeping
a low fraction of the targets,
we introduced 2 more symbols resulting in 4 symbols in total.
...
with Student's t-test. The confusion matrices were tested for independence
...
(p-values $< 0.0001$ with one-tailed Student's t-test).
...
(p-values $= 0.06$ with one-tailed Student's t-test).
...
Repeated measures ANOVA shows a significant difference between
all 4 groups: targets in offline, targets  in online,
non-targets in offline and non-targets in online, with p-value $< 0.0001$.
The post hoc analysis shows that it is driven by the difference between
...
for all methods
(p-values $< 0.05$ with Student's t-test after Benjamini-Hochberg correction
for 6 methods).
...
equals 0.37 and it is significantly different from the random level of 0.25 for
...
\cite{aydarkhanov_stability_2020}.

Finally, in this study, our primary goal was to explore the neural correlates
