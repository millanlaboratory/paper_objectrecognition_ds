...
\address{$^4$ZHAW Datalab, Zürich University of Applied Sciences, Winterthur, Switzerland
 }
...
\begin{abstract}
\textit{Objective.} In contrast to the controlled visual protocols and mental tasks,
visual recognition decoding during daily activities remains challenging.
The objective of this paper is to explore the
feasibility to decode such perceptual decisions in 
complex experimental conditions which demand a natural 
everyday behavior while interacting with dynamic content.
\textit{Approach.} 
...
The focus was put on Eye Fixation Related Potentials (EFRP)
in response to billboards suddenly appearing in the environment.
We decode the recognition of target vs non-target boards from 
EFRP in a closed-loop scenario. 
\textit{Main results.} By integrating decoding probabilities of multiple 
EFRP the accuracy reached 0.37 on average in identifying 
one target out of four board types. 
Using the acquired data, we performed a comparative study of 
classification algorithms and feature spaces
for offline and simulated online scenarios.
The EEG-derived approaches provide comparable 
above the chance-level performance between 0.53 and 0.6 AUC 
in binary classification of targets vs non-targets.
\textit{Significance.} These results show that visual recognition of sudden events
...
and key rehabilitation components for
...
to augment interaction for healthy users \cite{zhang_eeg-based_2015,khaliliardali_action_2015,chavarriaga_decoding_2018}. In this
...
In this work, we take a step further as we want to explore the
feasibility to decode perceptual decisions in 
natural experimental conditions where humans exhibit natural motor
behaviors while interacting with dynamic content. In particular,

%To bring value to able-bodied people 
%Brain-Computer Interface (BCI) systems should be smoothly 
%integrated in their daily activities.
%It is necessary, therefore, to successfully perform decoding
%of neural and behavioral correlates of targeted perceptual
%and/or cognitive processing while people actively interact with
%complex real-world environments. In contrast to the
%experimental conditions, neither 
%human behavior nor the content can be suppressed and controlled.
%In this line,
...
The explored conditions do not allow a direct translation
of the BCI technology into everyday life of healthy people.
...
is reflected in EEG as a well-known P300 component of Event-Related Potential (ERP).
...
For healthy users, however, this setup and typing performance does not bring much value.
...
and are manifested mainly in high-amplitude P100 or lambda component.
Later EEG deflections in EFRP resemble P300 component in the oddball paradigm \cite{brouwer_distinguishing_2013}.

Visual stimuli used in previous studies range from simple geometric shapes
...
Only a few attempts on ERP or EFRP decoding in videos or virtual reality (VR) simulations
...
The boards were invisible most of the time 
and suddenly appeared in the environment when the driver approached the them.
Such popping up of the board ensured that recognition
took place soon after the eye fixation.
Otherwise, there would be 2 challenges due to fixations on remote 
and poorly recognizable boards:
1) to locate the recognition timing of within long fixation,
...
and neural correlates of visual recognition and (2) to train
a classifier for the online phase.
...
The accurate decoding of the target traffic signs 
...
The eye gaze was recorded with SMI RED Eye tracking system with sampling rate of 120 Hz.
The chair and eye tracker positions were adjusted for each subject. The eye tracker
was calibrated with 13 points only once after the EEG setup and before beginning of 
the experiment.

The driving simulator logged the driver location,
the controllers state and the 2D position of boards on the screen at a sampling rate
...
which is applied in the online phase to provide the feedback on the decoding results.
...
The boards appear on a regular grid along the road, although
randomly on either side of the road with a maximum of 2 boards
...
as previously used in \cite{kamienkowski_fixation-related_2012}.
...
for training a classifier to be used in closed-loop the online phase
...
for the offline phase data and 
simpler and easy to implement for real-time processing
Identification by Dispersion-Threshold (IDT) \cite{salvucci_identifying_2000}
for the online phase and simulated online analysis.
...
\label{sec:fixanal}
The neural correlates of cognitive response are stronger when the stimulus is perceived and recognized
...
using non-causal filtering and downsampled from 2 kHz to 256 Hz.
...
PLR-Riemann classifier at this stage because the its training
...
and applied it in real time in online phase.
...
for 4 symbols. Accuracies were estimated per subject and averaged across 16 subjects.
...
we applied conservative Bonferroni correction. The average performance was tested
with Student's t-test. The confusion matrices were tested for independency
with Chi-square test separately for each subject with Bonferroni correction.
...
\begin{figure}[!t]
    ...
    targets vs distractors in offline and online phases.}
\label{fig:dwell}
\end{figure}
...
lead to the performance between 0.53 and 0.6 AUC on average
...
Eight out of thirteen subjects achieve performance above 0.6 for at least one of the approaches based
on EEG features.
...
real life interaction with machines for able-bodied users.
...
the cognitive process nor the EEG correlates.
...
At this stage we did not test the system for any specific application.
The achieved performance may be too low for critical task,
such as navigation,
so this system cannot be used to grant the full control to the vehicle.
However, it might be enough to recognize subjectively interesting
categories of road signs and object and provide valuable recommendations
to drivers.
