...
We report on gaze behavior and single-trial 
...
\textit{Main results.}
To identify the target out of four symbol types along a road segment, the BCI system integrated decoding probabilities of multiple EFRP and achieved the average online accuracy of 0.37 $\pm$ 6 (12 subjects), statistically significantly above the chance level.
Using the acquired data, we performed a comparative study
of classification algorithms (discriminating target vs. non-target) and feature spaces in
a simulated online scenario. The EEG approaches yielded
similar moderate performances not higher than 0.6 AUC,
yet statistically significantly above the chance level.
...
\center
    ...
    \caption{Visualization of the experimental setup and protocol.
        Top left: The experimental setup with the driving simulator (chair and 3 3D screens), 
        Eye-tracker and subject with EEG cap.
        Middle left: A screenshot from the beginning of the road with the cue on the floor
        and the first board on the left-hand side.
        Bottom left: A screenshot (in the online phase) showing the feedback after the finish line for the correct target symbol detection -- a green circle and the 2D board of the inferred target symbol. 
        Right: The schematic drawing of a road segment with the target cue, boards, indications left/right turn at the crossings and finish line. Note that the boards are invisible until they are approached to a close distance.
...
Subjects are instructed to search for boards with the target symbol and to pay attention to the feedback at the end of a road segment -- after crossing the finish line the inferred target symbol is rendered at the bottom of the screen as a 2D object (Figure \ref{fig:setup}). 
...
%the boards popped up at a regular pace with a 900 ms period.
...
In the offline phase, one of the two symbols were depicted on each board:
 \rotatebox[origin=c]{180}{$\exists$} and $\exists$.
...
To infer the target symbol at the end of the road each EFRP associated with the
...
The symbol that has the highest average probability of being a target class
...
The offline and online phases of our experiment have different objectives. As earlier explained, the offline phase is necessary for collecting good quality data for training a classifier which differentiates between attending target and non-target objects to be used in the closed-loop BCI in the online phase. However, it also allows us to study EEG correlates of visual recognition and perform a comparative study of different classification algorithms. The online phase allows the estimation of the closed-loop BCI application performance. In the online phase, we apply a chosen classifier trained on all the offline data. 

In order to make a comparative analysis of different decoding (classifications) approaches under online conditions, we performed a simulated online analysis. Namely, we trained different classifiers on all the offline data and evaluated their performance on the data recorded in the online phase. 
...
to match the value used for the IDT method (100 ms) and removal of downsampling
procedure due to lower sampling rate than in the original work.
...
in gaze analysis, we used IDT method to extract fixations.
...
%The cost function of the PLR is given in Eq.1, where y corresponds to the class label, $\mathbf{x}$ represents EEG features and $\beta$ the estimated coefficients.  

%\begin{equation}
    %\beta= arg min_{\beta} {|| y - \beta \mathbf{x}||}^2 + \lambda_2 {||\beta||}^2+\lambda_1 {||\beta||}_1
...
The group-level average performance was tested with non-parametric one-sided one-sample Wilcoxon signed-rank test against the theoretical chance level of 0.5 AUC for each classifier.
...
This subject is excluded from the gaze and other analysis.
...
%paired t-test providing $p = 0.04$.
...
%paired t-test providing $p = 0.002$.
...
confusion matrix.
The reason is that for these two subjects the accuracy of the symbol $\Xi$ was higher and the accuracy of the symbol E was lower than of the other symbols.
...
This can be explained by the effect of the relative motion of user/driver to the objects on
...
which suggests
that peripheral recognition is not completely excluded by the stimuli design.
...
we did the comparative study using the recorded online data (simulated online analysis).
...
decoders did not exhibit the improvement similar to the classification models based on
dwell time. 
...
The approaches used in online phase proved to be time efficient without
...
these steps are the least demanding in terms of computational time
...
considerable amount of time especially when applied multiple
times within nested cross validation. The most demanding
steps included estimation of parameters for Riemannian features
and for Random Forest classifier. This led to an extended
pause between offline and online phases of the experimental session.
...
trials in terms of stimuli and subjects’ behavior.
...
By decoding the brain signals the intelligent car can learn
drivers’ interest and enhance their interaction by
creating a tailored recommendation to them.
...
promising for bringing this type of BCIs to intelligent cars.
