...
the feasibility of decoding the EEG signature 
...
EFRP decoding performance (fixations on visually similar target vs. non-target objects).
In addition, we demonstrate the application
of our approach in a closed-loop BCI setup.
\textit{Main results.}
To identify the target out of four symbol types along a road segment, the BCI system integrated decoding probabilities of multiple EFRP and achieved the average online accuracy of 0.37 $\pm$ 0.06 (12 subjects), statistically significantly above the chance level.
...
similar moderate performances of at most 0.6 AUC,
...
Therefore, this study lays a foundation for assistive and recommender systems
based on the driver's brain signals.
\end{abstract}
...
\cite{ramos-murguialday_brain-machine_2013,pichiorri_braincomputer_2015,biasiucci_brain-actuated_2018,cervera_braincomputer_2018}
...
The experiment consisted of two phases: offline and online.
...
\center
    ...
    \caption{Visualization of the experimental setup and protocol.
        Top left: The experimental setup with the driving simulator (car seat, steering wheel, pedals and 3 3D screens), 
...
to poor fixation extraction. This participant was excluded from the analyses.
...
and all the participants provided written consent.
...
Each detected fixation was assigned to one of the three categories:
a target board fixation, a non-target board fixation, or a non-board fixation.
Considering that 
(i) the visual span (the angular span) within which our vision is sharp enough to perform a given task extends up to several degrees ($\thicksim 3-10 \degree$ in central vision) \cite{legge_psychophysics_2001},
(ii) the driver's motion relative to the boards imposes visual tracking to perform the search task,
and (iii) the noise in eye-tracking (sudden and brief disruptions spanning several data samples are occasionally observed in the eye-tracking data), we applied the following approach to assigning the boards to fixations. 
...
with Pearson's chi-square test \cite{pearson_x_1900} separately for each subject 
...
\center
    ...
    \includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=0.9\columnwidth]{../images/offline/TopoPlot_signRSquare_offline.pdf}
    \includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=0.9\columnwidth]{../images/online/TopoPlot_signRSquare_online.pdf}
